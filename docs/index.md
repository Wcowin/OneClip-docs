...
- **🔎 极速搜索**：随打随搜，多维筛选快速定位
- **🗂️ 全格式支持**：支持 9+ 格式包括文本、图片、视频、音频、文档、代码、压缩包等，完整保留元数据
- **⌨️ 全局快捷键**：`⌘+⇧+V` 呼出主界面，支持自定义组合
- **💬 快捷回复**：`⌘+⇧+R` 呼出快捷回复界面，支持文本/图片/文件模板
- **🎨 布局多样**：支持列表/卡片双模式切换，卡片模式支持三种方向
- **📥 栈粘贴板**：`⌘+⇧+C` 呼出栈粘贴板，`⌘+V` 依次粘贴栈内容，`⌘+⇧+S` 按行拆分捕获
- **📦 拖拽容器**：`⌘+⇧+D` 呼出拖拽容器，暂存并拖出文件/图片到其他应用
- **⚡ 快速粘贴面板**：`⌘+；` 呼出快速粘贴面板，1-9 快速粘贴历史记录
- **🧀 OCR 识别**：支持屏幕/图片内容识别，OCR 翻译，图片自动 OCR 可搜索图片中文字
- **🤖 AI 集成**：支持本地 AI（Ollama/LMStudio）和在线 AI 服务，智能总结与翻译
- **🌐 局域网同步**：同一 WiFi 下 Mac 与手机/Mac 实时双向同步，无需安装 App，浏览器即可使用
- **☁️ 云同步**：支持 Syncthing 和自定义 iCloud 等多设备同步
- **🔒 隐私保护**：支持密码保护、Touch ID/Face ID 生物识别验证
- **📱 设备管理**：多设备许可证管理，无限同时使用设备，灵活暂停和恢复
- **📝 脚本功能**：支持 JavaScript 脚本编写，自动处理剪贴板内容，支持链式执行
- **👍 访达增强**：支持访达 `⌘+X` 剪切文件，然后 `⌘+V` 移动文件
- **🎯 智能分类**：文本、图片、文件、链接、代码等自动识别分类
...
### 6️⃣ 贴图/便签 - 悬浮窗口（v1.5.7+）

- `⌘+⇧+P` 贴图窗口，悬浮显示剪贴板内容
- `⌘+⇧+N` 便签窗口，快速记录临时信息
- 支持拖拽、缩放等操作

### 7️⃣ PopClip 选中菜单（v1.5.7+）
